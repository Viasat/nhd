...
$(PROTO_OBJ)%_pb2.py \
$(PROTO_OBJ)%_pb2_grpc.py : $(PROTO_SRC)%.proto $(MAKEFILE)
		$(ECHO) Processing [$<] to [$@]
		$(PROTOC) -I $(PROTO_SRC) --python_out=$(PROTO_OBJ) --grpc_python_out=$(PROTO_OBJ) $<

$(PROTO_OBJ)__init__.py:
		$(ECHO) [$@]
		$(MKDIR) $(PROTO_OBJ)
		$(ECHO) "import sys" >> $@
...
GRPC_AUTOGEN_FILES += $(PROTO_OBJ)__init__.py
GRPC_AUTOGEN_FILES += $(patsubst $(PROTO_SRC)%.proto, $(PROTO_OBJ)%_pb2.py, $(foreach SUBDIR, $(PROTO_SRC), $(FIND_GRPC_PROTO_FILES)))
GRPC_AUTOGEN_FILES += $(patsubst $(PROTO_SRC)%.proto, $(PROTO_OBJ)%_pb2_grpc.py, $(foreach SUBDIR, $(PROTO_SRC), $(FIND_GRPC_PROTO_FILES)))
...
# GRPC Stuff
FIND_GRPC_PROTO_FILES = $(wildcard $(SUBDIR)*.proto)
GRPC_PROTO_FILES := $(foreach SUBDIR, $(PROTO_SRC), $(FIND_GRPC_PROTO_FILES))
...
# NHD Wheel
#**********************************************************************
NHD_WHEEL_DEPENDS = $(GRPC_AUTOGEN_FILES) $(NHD_FILES) setup_template.py nhd-version bin/nhd
$(NHD_WHEEL): $(MAKEFILE) $(NHD_WHEEL_DEPENDS)
	$(RM) -r $(TMP)bdist
	$(RM) -r $(BUILD_DIR)
...
$(TMP)docker: $(DOCKER_DEPENDS) $(NHD_WHEEL)
